<script>
	...
	import Anagram from '$lib/Anagram.svelte';
	...
	function onSort(e) {
		...
		if (movesLeft === 0) {
			const missedWords = missingWords()
				.map((w) => `&ldquo;${w.name}&rdquo;`)
				.join(', ');
...
<main>
	...
	<div class="action">
		...
		{:else if wordsFound.length > 0}
			<button class="button" on:click={loadLevel}> Next Level </button>
		{/if}
	</div>
	...
	<div class="bottom-ui">
		...
		{#if hints > 0 && wordsFound.length < possibleWords.length}
			<button class="button button--small" on:click={useHint}> Use a Hint </button>
		{/if}
